...
void MCMF_CS2::allocate_arrays()
{
	...
	//what's the function of _arc_first and _arc_tail?
						...
						price_t cost)
{
	...
	// no of arcs incident(¶Ë) to node i is placed in _arc_first[i+1] 
	// _arc_first[i+1] store the in degree of node i
	_arc_first[tail_node_id + 1] ++; 
	_arc_first[head_node_id + 1] ++;
	_i_node = _nodes + tail_node_id;// _i_node is the NODE pointer 
...
/************************************************************************
update _epsilon _cut_off _cut_on .
************************************************************************/
int MCMF_CS2::update_epsilon()
{
	...
	_epsilon = (price_t) (ceil ( (double) _epsilon / _f_scale ));//decrease epsilon
	_cut_off = _cut_off_factor * _epsilon;//_cut_off_factor==12.0
	_cut_on = _cut_off * CUT_OFF_GAP;//CUT_OFF_GAP==0.8
...
void MCMF_CS2::cs_cost_reinit()
{
	...
	_epsilon = ceil((long double)(sum / _dn));
...
/************************************************************************
calcaulate the min-cost according to updated arcs
************************************************************************/
void MCMF_CS2::finishup( double *objective_cost)
{
	...
	// (2)calculate the cost of feasible flow
	for ( a = _arcs, na = 0; a != _sentinel_arc ; a ++, na ++ ) {
		cs = a->cost() / _dn; //_dn==_n+1
		//flow: actual flow on arc a
		if ( _cap[na]  > 0 && (flow = _cap[na] - a->rez_capacity()) != 0 )
			obj_internal += (double) cs * (double) flow;
		a->set_cost( cs);//set actual arc cost
	}
...
void MCMF_CS2::cs2( double *objective_cost)
{
	...
	// (1) update epsilon first (initial epsilon);
	update_epsilon();


	// (2) scaling loop;
	do {
		refine();//update f and p
		...
		while (1) {
			...
			if ( _n_ref >= PRICE_OUT_START ) {
				...
				//cc=update_epsilon==1 exit the while circle
...
int MCMF_CS2::run_cs2()
{
	...
	// () PRINT_ANS?
	if ( _print_ans == true ) {
		//modify by wjs 0517 ,comment the print_solution() 
		print_solution();
	}
