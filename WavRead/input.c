...
*Return:PCM_FILE 文件结构指针
*Other:
******************************************************************************/

PCM_FILE *WavOpenRead(const char *name, int rawinput)
{
  int i;
  int skip;
  FILE *wave_f;
...
size_t WavReadFloat32(PCM_FILE *sndf, float *buf, size_t num, int is_normalized)//0:读取非归一化浮点数据 1：归一化浮点数据
{
  size_t i = 0;
